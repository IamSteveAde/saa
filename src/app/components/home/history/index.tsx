...
function getBudgetLabel(value: number, type: BusinessType): string {
  const max =
    type === "established" ? ESTABLISHED_BUDGET_MAX : EMERGING_BUDGET_MAX;
  if (value >= max) return `${formatNaira(value)}+`;
  return formatNaira(value);
}

function getBudgetSteps(type: BusinessType): number[] {
  if (type === "established") {
    return [500000, 750000, 1000000, 1500000, 2000000, 3000000, 5000000];
  }
  return [200000, 300000, 400000, 500000, 750000, 1000000, 2000000];
}
...
export default function Contact() {
  ...
  const budgetPct =
    form.budget > 0
      ? ((form.budget - budgetMin) / (budgetMax - budgetMin)) * 100
      : 0;

  const setType = (type: BusinessType) => {
    const min =
      type === "established" ? ESTABLISHED_BUDGET_MIN : EMERGING_BUDGET_MIN;
    setForm((f) => ({ ...f, businessType: type, budget: min, services: [] }));
  };

  const handleBudgetChange = (raw: number) => {
    const snapped = snapToStep(raw, budgetSteps);
    setForm((f) => ({ ...f, budget: snapped }));
  };
 ...
 // Replace the existing handleSubmit function in Contact.tsx with this:
...
const handleSubmit = async () => {
      ...
      <section className="relative overflow-hidden bg-white py-32 px-6 md:px-20">
        <div className="mx-auto flex max-w-2xl flex-col items-center gap-8 text-center">
          ...
          <div>
            ...
            >
              Thank you, {form.name.split(" ")[0]}. Your enquiry has been
              received. A member of the Spotlite Africa team will contact you
              within 24 hours.
            </p>
          </div>
          <p
  ...
  return (
    <section
      className="relative overflow-hidden bg-white py-24 px-6 md:px-20"
      id="contact"
    >
      {/* subtle dot grid */}
      <div
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        ...
        <motion.div {...fadeUp(0)} className="mb-16 max-w-2xl">
          ...
          <p
            ...
            style={{ color: "rgba(0,0,0,0.45)" }}
          >
            We review every enquiry personally. If there is a fit, we will reach
            out within 24 hours to schedule a private consultation.
          </p>
        </motion.div>
          ...
          <motion.div {...fadeUp(0.05)}>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {(
                ...
                ] as { id: BusinessType; title: string; desc: string; bullets: string[] }[]
              ).map((option) => {
            ...
            {form.businessType && (
              ...
              >
                ...
                <div className="mt-6 rounded-2xl p-8"
                  style={{ border: "1.5px solid rgba(0,0,0,0.08)", background: "#FAFAF9" }}>
                  ...
                  <div className="mb-8 flex items-end justify-between">
                    <div>
                      ...
                      <p
                        className="mt-1 text-4xl font-medium tracking-tight md:text-5xl"
                        style={{ color: "#0A0A0A" }}
                      >
                        {getBudgetLabel(form.budget, form.businessType)}
                        <span
                          className="ml-1 text-base font-light"
                          style={{ color: "rgba(0,0,0,0.3)" }}
                        >
                          /month
                        </span>
                  ...
                  <div className="relative">
                    ...
                    {/* thumb */}
                    <div
                      className="pointer-events-none absolute top-1/2 -translate-y-1/2 -translate-x-1/2 transition-all duration-150"
                      style={{ left: `${budgetPct}%` }}
                    >
                      <div
                        className="h-5 w-5 rounded-full shadow-md"
                        style={{ background: "#0A0A0A", border: "3px solid #F5C842" }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex justify-between">
                    {budgetSteps.map((step) => (
                      <button
                        key={step}
                        onClick={() => setForm((f) => ({ ...f, budget: step }))}
                          ...
                          style={{
                            background:
                              form.budget === step
                                ...
                                : "rgba(0,0,0,0.2)",
                            transform:
                              form.budget === step ? "scale(1.6)" : "scale(1)",
                          }}
                        />
                        <span
                  ...
                  {/* context note */}
                  <p
                    className="mt-6 text-xs font-light leading-relaxed"
                    style={{ color: "rgba(0,0,0,0.35)" }}
                  >
                    {form.businessType === "established"
          ...
          <AnimatePresence>
            {form.budget > 0 && (
              <motion.div
                ...
                transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
              >
                <StepLabel number="03" label="Your details" />
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                  {(
                    ...
                    ] as { key: keyof FormData; label: string; type: string; placeholder: string }[]
                  ).map((field) => (
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  {services.map((service) => {
                    ...
                    return (
                      ...
                      >
                        {active && (
                          <span style={{ color: "#F5C842", marginRight: 6 }}>✓</span>
                        )}
                        {service}
                      </button>
                    );
            ...
            {form.services.length > 0 && (
              ...
              >
                <StepLabel
                  ...
                  sub="We will confirm within 24 hours"
                />
                  ...
                  {AVAILABILITY_OPTIONS.map((option) => {
                    ...
                    return (
                      <button
                        key={option.value}
                        onClick={() =>
                          setForm((f) => ({ ...f, availability: option.value }))
                        }
                      ...
                      >
                        <span
                          ...
                          style={{ color: active ? "#F5C842" : "#0A0A0A" }}
                        >
                          {option.label}
                        </span>
                        <span
          ...
          <AnimatePresence>
            {canSubmit && (
              ...
              >
                ...
                >
                  <p
                    className="mb-4 text-[10px] font-medium uppercase tracking-[0.28em]"
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {[
                      ...
                      },
                      {
                        ...
                        value: getBudgetLabel(form.budget, form.businessType),
                      },
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {form.services.map((s) => (
                          <span
                            key={s}
                            className="rounded-full px-3 py-1 text-[11px] font-light"
                            style={{
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <p
                    ...
                    style={{ color: "rgba(0,0,0,0.35)" }}
                  >
                    We review every enquiry personally. You will hear from us
                    within 24 hours if there is a fit.
                  </p>
